...
JsKoansReporter.prototype.reportRunnerResults = function(runner) {
  ...
  if (this.failedSpecs === 0) {
    status = 'passed';
    enlightenmentMessage = "悟りを得た!";
  } else {
    status = 'failed';
    enlightenmentMessage = "悟りはまだまだ遠い...";
  }
...
JsKoansReporter.prototype.reportSpecResults = function(spec) {
  ...
  if ( !results.passed() ) {
    ...
    description = spec.description + "。カルマを清算してください。"
  } else {
    description = spec.description + "について気づきを得ました。"
  }

  var specDiv = this.createDom('div', { className: 'spec '  + skipStatus },
      this.createDom('a', { className: 'run_spec_' + status, href: '?spec=' + encodeURIComponent(spec.getFullName()) }, "再び瞑想する"),
      this.createDom('a', {
  ...
  for (var i = 0; i < resultItems.length; i++) {
    ...
    if (result.type == 'expect' && result.passed && !result.passed()) {
      ...
      messagesDiv.appendChild(
          this.createDom('div', { className: 'description' }, "以下のコードについて瞑想してください:")
          );
...
JsKoansReporter.prototype.getSuiteDescription = function(suite) {
  if (null === suite.parentSuite) {
    return suite.description + "について考える";
  } else {
    return suite.description + "について";
  }
};
